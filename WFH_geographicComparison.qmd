...
data2021 <- data2021 %>% select(YEAR, INCEARN, INCWAGE, INCTOT, TRANWORK, OCCSOC, CLASSWKR, EMPSTAT, LABFORCE, PERWT, COUNTYFIP, PUMA, PWSTATE2, AGE, STRATA, CLUSTER, RACE, SEX, CIHISPEED, CINETHH, MULTGEN, NCHILD, NCHLT5, MARST, FERTYR )
...
data2019 <- data2019 %>% select(YEAR, INCEARN, INCWAGE, INCTOT, TRANWORK, OCCSOC, CLASSWKR, EMPSTAT, LABFORCE, PERWT, COUNTYFIP, PUMA, PWSTATE2, AGE, STRATA, CLUSTER, RACE, SEX, CIHISPEED, CINETHH, MULTGEN, NCHILD, NCHLT5, MARST, FERTYR )
...
table <- #dstrata %>% factor(CIHISPEED,)
  svytable(~CIHISPEED +did_wfh_labels+YEAR, design = dstrata) 
...
table %>% ggplot(aes(factor(CIHISPEED, labels = c("Has Access", "No Access")), y=Prop, fill = did_wfh_labels, group = YEAR)) + 
  geom_col(stat = "identity", position = "stack") +
  facet_wrap(~YEAR)+
    geom_text(aes(label = scales::percent(Prop)), position = position_fill(vjust=.5), size=3) + 
  theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
...
mapPUMAboth %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black")+ labs(title = "Percent of PUMA area population that did work from home in 2021", subtitle = "Each PUMA has >100K & < 200K people", caption = "Facet_wrapped by year") + 
  facet_wrap(~YEAR)



PUMAmapdata2019 <- #dstrata2019 %>%
  joined %>% 
  as_data_frame() %>%
...
PUMAmapdata2021 %>%
  filter(did_wfh_labels == "Did WFH") %>%
  ggplot(aes(fill = pct_weight)) +
  geom_sf(aes(geometry = geometry), color = "black")+ labs(title = "Percent of PUMA population that did work from home in 2021", subtitle = "Compare to face_wrap graphs", caption = "Note:")
...
mapPUMAboth %>%
  ...
  labs(title = "Percent of PUMA area population that had access to Internet", subtitle = "Each PUMA has >100K & < 200K people", caption = "Facet_wrapped by year") + 
  facet_wrap(~YEAR)
...
mapPUMAboth %>%
  filter(CIHISPEED != 20 ) %>% 
  ggplot(aes(fill = pct_weight)) +
  geom_sf(aes(geometry = geometry), color = "black")+ labs(title = "Percent of PUMA area population that had access to Internet", subtitle = "Each PUMA has >100K & < 200K people", caption = "Facet_wrapped by year") + 
  facet_wrap(~YEAR)
