...
#svyglm(did_wfh~ INCEARN +race_cat+ SEX+ AGE + CIHISPEED*CINETHH + CanWorkFromHome+county_pop_type+NCHILD+MARST + NCHLT5, design = dstrata2019) %>% summary()
...
```{r Figure5a, fig.cap = "The did_WFH variable is based on TRANWORK==80 from ACS 2019 & 2021 1-Year Survey individual level data. Each geographic region has between 100K and 200K people."}
...
mapPUMAboth %>% pivot_wider(id_cols = -c(n), names_from ="YEAR", values_from = "Prop") %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black") + 
  theme_void()+ 
   scale_fill_steps2(high ="red4", mid= "white", low = "forestgreen", 
                       n.breaks = 5,
                     na.value=NA,
                     nice.breaks=FALSE,
                     show.limits = TRUE,
...
## Model 0: Both years together

```{r}


both <- svyglm(did_wfh~ INCEARN +race_cat+ factor(SEX)+ age_cat +# CIHISPEED*CINETHH +
                    CanWorkFromHome+county_pop_type+ NCHILD + NCHLT5 + occ_2dig_labels + factor(YEAR), 
...
t1 <- both %>%
  ...
  modify_caption("2019 & 2021 ACS Data")
...
m1_2019 <- svyglm(did_wfh~ INCEARN +race_cat+ factor(SEX)+ age_cat +# CIHISPEED*CINETHH +
                    CanWorkFromHome+county_pop_type+ NCHILD + NCHLT5 + occ_2dig_labels, 
        family = quasibinomial(),design = dstrata2019)

m1_2021<- svyglm(did_wfh~ INCEARN +race_cat+ factor(SEX)+ age_cat + #CIHISPEED*CINETHH + 
                   CanWorkFromHome + county_pop_type + NCHILD + NCHLT5+ occ_2dig_labels, 
      family = quasibinomial(), design = dstrata2021)
  
export_summs(m1_2019, m1_2021, 
             model.names = c("M12019 QuasiBi", "M12021 QuasiBi"), 
            robust = "HC3", statistics = c(N = "nobs", R2 = "r.squared", adjR2 = "adj.r.squared"))
...
```{r include = FALSE, eval=FALSE}
m1_2019 <- svyglm(did_wfh~ INCEARN +race_cat+ SEX+ AGE + CIHISPEED*CINETHH + CanWorkFromHome+county_pop_type+NCHILD + NCHLT5, 
       design = dstrata2019)

m1_2021<- svyglm(did_wfh~ INCEARN +race_cat+ SEX+ AGE + CIHISPEED*CINETHH + CanWorkFromHome+county_pop_type+NCHILD + NCHLT5, 
       design = dstrata2021)
  

export_summs(m1_2019, m1_2021, 
             model.names = c("Linear WRONG 2019", "Linear WRONG 2021"), 
            robust = "HC3", statistics = c(N = "nobs", R2 = "r.squared", adjR2 = "adj.r.squared"))
...
```{r}
# m3_2019 <- svyglm(did_wfh~occ_2dig_labels*CanWorkFromHome+race_cat+ SEX+ age_cat +CIHISPEED*CINETHH + county_pop_type+NCHILD*NCHLT5+ MARST, design = dstrata2019)
# m3_2021 <- svyglm(did_wfh~occ_2dig_labels*CanWorkFromHome+race_cat+ SEX+ age_cat +CIHISPEED*CINETHH + county_pop_type+NCHILD*NCHLT5+ MARST, design = dstrata2021)# %>% summary()
# 
...
m4_2019 <- svyglm(did_wfh~NCHLT5+factor(SEX) + occ_2dig_labels+county_pop_type, family = quasibinomial(), design = dstrata2019 )# %>% summary()

m4_2021 <- svyglm(did_wfh~NCHLT5+factor(SEX) + occ_2dig_labels+county_pop_type, family = quasibinomial(), design = dstrata2021)# %>% summary()

export_summs(m4_2019, m4_2021, 
             model.names = c("M4 2019 QuasiBi", "M4 2021 QuasiBi"), 
...
Decreases liklihood of working from home: Lacking access to internet, WFH not feasible & occupation labels within Production and Transportation,
