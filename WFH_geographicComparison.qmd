...
data2021 <- data2021 %>% select(YEAR, INCEARN, INCWAGE, INCTOT, TRANWORK, OCCSOC, CLASSWKR, EMPSTAT, LABFORCE, PERWT, COUNTYFIP, PUMA, PWSTATE2, AGE, STRATA, CLUSTER, RACE, HISPAN, SEX, CIHISPEED, CINETHH, MULTGEN, NCHILD, NCHLT5, MARST, FERTYR, EDUC, DEGFIELD, OCC, IND, OCC2010 )
...
data2019 <- data2019 %>% select(YEAR, INCEARN, INCWAGE, INCTOT, TRANWORK, OCCSOC, CLASSWKR, EMPSTAT, LABFORCE, PERWT, COUNTYFIP, PUMA, PWSTATE2, AGE, STRATA, CLUSTER, RACE, HISPAN, SEX, CIHISPEED, CINETHH, MULTGEN, NCHILD, NCHLT5, MARST, FERTYR, EDUC, DEGFIELD, OCC, IND, OCC2010 )
...
occ_2digits %in% c(45,46,47,48,49 ) ~"Nat. Res., Constr., Maintenance",
...
data <- data %>%
 ...
 occ_2digits %in% c(11) ~ "Management",
  ...
  occ_2digits %in% c(13) ~ "Bus. & Fin.",
  occ_2digits %in% c(15) ~ "Computer, engineering & science",
  occ_2digits %in% c(17) ~ "Architecture & Eng.",
  occ_2digits %in% c(19) ~ "Life/Social Sciences",
 occ_2digits == 21 ~ "Comm & soc. Services",
occ_2digits == 23 ~ "Legal",
occ_2digits == 25 ~ "Edu. Instruc.",
...
occ_2digits == 49 ~"Install., Maintenance",
...
table %>% ggplot(aes(x=fct_rev(fct_inorder(occ_2dig_labels)), y=n, group = YEAR)) + 
  ...
  labs(title ="Percent of people in each Major Occupation Group",
       x = "", y = "# of People",
...
table %>% ggplot(aes(x=fct_rev(fct_inorder(occ_2dig_labels)), y=n, fill = did_wfh_labels, group = YEAR)) + 
  geom_col(stat = "identity", position = "stack") +
  facet_wrap(~YEAR)+
  #  geom_text(aes(label = scales::percent(Prop, accuracy = .1L)), position = position_stack(vjust=.5), size=3) + 
  ...
  theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
  labs(title ="Percent working from home by Broad Occupation type",
       x = "", y = "# of People",
      caption = "ACS 1 year samples for 2019 and 2021 used for weighted population estimates,") + scale_y_continuous(labels = scales::comma)+
...
theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
  labs(title ="Percent of Workers that Could Work From Home by Occupation, 2019 & 2021", subtitle = "OCCSOC codes and Teleworkable scores from occupation characteristics",
       x = "", y = "# of People",
      caption = "ACS 1 year samples for 2019 and 2021 used for weighted population estimates,") + scale_y_continuous(labels = scales::comma)+
...
table %>% #filter(YEAR == 2021) %>%
  ...
  geom_text(aes(label = scales::percent(as.numeric(ifelse(Prop>0.01,Prop, "")), accuracy = .1), accuracy= 0.1L ), position = position_stack(vjust=.5), size=3) +
theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
  labs(title ="Percent of Workers that Could Work From Home in 2021", 
       x = "", y = "# of People",
...
# Matches Xiaoyan's table from meeting on May 9th
crosstab_3way(dstrata, CanWorkFromHome, did_wfh_labels, YEAR,  weight = PERWT, pct_type = "cell")
...
joined %>% crosstab_3way(x=YEAR, y=did_wfh, z=black, weight = PERWT) %>% 
  ggplot(aes(x=`1`, y=YEAR, fill = black)) + 
  geom_col(stat="identity", position = "fill")
...
mapPUMAboth %>%
  ...
  labs(title = "Percent of PUMA area population that did work from home", 
       subtitle = "2019 vs 2021", 
       caption = 
         "The did_WFH variable based on TRANWORK==80 from ACS 2019 & 2021 1-Year Survey 
       individual level data. Each geographic region has between 100K and 200K people. ") +
  theme_classic() + 
...
mapPUMAboth %>% 
  ...
  scale_fill_gradientn(colors = c("red4", "white", "forestgreen"),
                      ...
                      # values = c(-.15,-.03,0,0.03,.15),  
                       na.value = "grey",
                       n = 9,
                        name = "Change in Access at home", label = scales::percent)
```
...
mapPUMAboth %>%
  ...
  facet_wrap(~YEAR)+
 scale_fill_gradientn(colors = c("white", "forest green"), 
                       n = 5,
                        name = "Population %", label = scales::percent)
...
mapPUMAboth %>%
 ...
 scale_fill_gradientn(colors = c( "white", "red4"), 
                       n = 5,
                        name = "Population %", label = scales::percent)#mapPUMAboth <- mutate(pct_change = )
...
mapPUMAboth %>% pivot_wider(id_cols = -c(n), names_from ="YEAR", values_from = "Prop") %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black")+ 
    theme(legend.title = element_blank())+
 # scale_fill_distiller(palette = "RdYlGn", direction = -1)+
   scale_fill_gradientn(colors = c("red4", "white", "green4"), 
                       n = 7,
                        name = "Population %", label = scales::percent)+
  labs(title = "Change in access to High Speed Internet")
```
...
```{r}
svyglm(did_wfh~race_cat+SEX+CIHISPEED*CINETHH + occ_2dig_labels*CanWorkFromHome+county_pop_type, design = dstrata) %>% summary()
```
