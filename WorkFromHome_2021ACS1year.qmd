...
1.  INCEARN = INCWAGE + INCBUS00
    -   Total amount earned in last 12 months: Wages, salary, commissions, bonuses, tips. \[Yes --\> \_\_\_\_\_\_ \] is coded as INCWAGE value.
        ...
        -   INCWAGE is topcoded only at the 99.5th percentile with a state. So less than 1% of incomes will be coded as a lower value. [Source](https://usa.ipums.org/usa-action/variables/INCWAGE#editing_procedure_section). Not a problem.

    -   Total amount earned in last 12 months: Self-employment income from own businesses (includes farms, nonfarms, proprietorship, and partnerships). \[Yes --\> \$\_\_\_\_\] is coded as INCBUS00 value.

        -   Note: I somehow missed downloading INCBUS00 in the IPUMS extract but we should not need it and it doesn't seem worth re-downloading all the extracts in different formats and re-adding them to box.
2.  INCEARN includes self-employment income, INCWAGE does not.
...
-   Valid percent: 20.5% of observations with responses did WFH and 79.4% of observations with responses did not WFH. **This is the statistic we would use**
...
CrossTable(dataXH$did_wfh, dataXH$has_incearn)

xtabs(has_occsoc~has_incearn, data = dataXH) #49K + have OCCSOC codes


table(dataXH$did_wfh, dataXH$has_occsoc)


CrossTable(dataXH$did_wfh_labels, dataXH$has_occsoc_labels)
...
t.test(INCEARN~did_wfh, data = dataXH)
# statistically significant difference between those that did and did not work from home in their earned wages

### Chi-squared

chisq.test(dataXH$did_wfh_labels, dataXH$SEX)
```
...
# practice test to see if sex is 52% male and 48% female. 
chisq.test(table(dataXH$SEX), p = c(52, 48)/100)
...
```{r summarytables-check, eval=FALSE}
