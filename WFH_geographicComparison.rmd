...
table %>% 
  ...
  ggplot(aes(CanWorkFromHome, y=n, 
             fill = did_wfh_labels, group = YEAR)) + 
  geom_col(stat = "identity", position = "stack") +
 
 geom_text(aes(label = scales::percent(as.numeric(ifelse(Prop > 0.02, Prop, "")), accuracy = .1) ), position = position_stack(vjust = .5), size=3
               ) +
  #geom_text(aes(#x=CanWorkFromHome, y=sum(n), 
             #   label = scales::percent(as.numeric(ifelse(Prop > 0.02, Prop, "")), accuracy = .1) ), 
          # ), 
          #  size=3
           # ) +
  #scale_fill_manual(values = c("#a6bddb", "#2b8cbe"))+
 ...
 #      x = "", y = "Estimated Number of Workers") + 
  scale_y_continuous(labels = scales::comma) + 
  scale_fill_manual(values = c("#a6bddb",  "#2b8cbe"))+ facet_wrap(~YEAR)

#ggsave("Figure1.eps", limitsize = FALSE,width = 8, height = 4, units = "in")
#ggsave("Figure1.pdf", limitsize = FALSE,width = 8, height = 4, units = "in")
ggsave("Figure7-didWFHcanWFH.png", limitsize = FALSE, width = 7, height = 4, units = "in")
```

```{r figure7-alt}
table <- svytable(~CanWorkFromHome+YEAR+did_wfh_labels, design = dstrata) 
# proportion of each respondent's sex and if they worked from home for each year in sample
table <- table %>% 
  as_tibble() %>% 
