---
title: "Alea's Expanded Occupation List"
subtitle: "Based on Replication of Dingel & Neiman 2020 WFH Feasibility"
...
## Keeps only Can WFH and Cannot WFH. Drops "Some WFH"
# goes from 123,753 observations to 107,617 observations.
# some WFH = ~15,000 observations. 
joined <- joined %>% filter(teleworkable == 0 | teleworkable==1)

#valid percent for how many did WFH in 2019
joined %>% filter(YEAR == 2019) %>%
topline(did_wfh_labels, weight = PERWT)

# valid percent of how mnay people did work from home for 2021
...
My expanded version has 1298 potential occupation codes.

67,339 are 0's (Cannot work from home)\
40,278 are 1's. (Feasible work from home).

16,136 are aggregated values made from multiple detailed 8-digit occupation codes --\> 6-digit occupation codes OR 4- or 5- digit OCC codes that end with "00" or "XX" used to protect identities for occupations with few workers. These occupation codes that end with XX are not included in D&N's paper. In this version of the code, I use my expanded occupation codes that include more potential aggregated or de-identified occupations so that more observations are kept in the sample but exclude the "Some WFH" category.

# Descriptive Statistics

123,753 observations originally in ACS 2021 and 2019 1-year samples.

107,000 observations after removing those not in the labor force and with earned incomes less than or equal to zero AND after removing "Some WFH" feasibility occupations.
...
-   **Valid percent:** 16.3% did WFH in 2021. This is the statistic we would use. Increased from 4.5% of labor force over the age of 16 that did work from home in 2019.
...
-   40,278 observations have occupations that indicate that they could Work from home
-   67,339 responses have occupations indicating that they can not work from home
-   [DROPPED: 16,136 observations have occupation codes that made from multiple occupations that have a mixture of work from home feasibility. These will be dropped in this version of the code.]{.underline}
...
dstrata <- survey::svydesign(id = ~CLUSTER, strata = ~STRATA, weights = ~PERWT, data = joined)# %>% as_survey() 
  #mutate(decile = ntile(INCEARN, 10))


#dstrata %>% group_by(PUMA, CanWorkFromHome) %>% summarize(unw_count = unweighted(n()))
...
Changes from adding my OCC Codes ) without the "Some WFH" teleworkable codes:

**Increases** from adding my extra occupation codes: Management (9.6 to 11.6) Office & Admin (9.0 to 10.8)

**Decreases** in overall makeup of workforce: Transpo and Material moving become a smaller share of the workforce after adding my additional occupation codes. Other categories have some small changes.

> TO DO: Look at the codes I added: are they mostly management jobs? Are they new categories that didn't exist when D&N wrote their paper?

Joined dataframe has 54,499 observations in 2019 and 53,118 observations in 2021.

```{r}
#| code-fold: true

#table includes observations from BOTH years for the 5 broadest occupation categories.
table(joined$occ_2dig_labels, joined$did_wfh_labels)

# has 22 or 23 categories created from first 2 digits of OCCSOC code.
...
table %>% filter(occ_2dig_labels != "Military") %>% 
  ggplot(aes(x=fct_rev(fct_inorder(occ_2dig_labels)), y=n, group = YEAR)) + 
...
table <- table %>% 
  ...
  group_by(YEAR)%>% 
  mutate(Prop =round(n/sum(n), digits=3)) %>%
  arrange(did_wfh_labels, desc(n))
table

table %>%ggplot(aes(x=fct_rev(fct_inorder(occ_2dig_labels_d)), y=n, fill = factor(did_wfh_labels, levels = c("Did WFH", "Did not WFH")), group = YEAR)) + 
  geom_col(stat = "identity", position = "stack") +
  facet_wrap(~YEAR)+
...
theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
  ...
  scale_fill_manual(values = c( "#66CC99","#66CC00","gray")) +
  coord_flip()
...
table %>% 
  ...
  geom_text(aes(label = scales::percent(as.numeric(ifelse(Prop>0.01,Prop, "")), accuracy = .1), accuracy= 0.1L ), position = position_stack(vjust=.5), size=3) +
theme_classic() + theme(legend.position = "bottom", legend.title = element_blank())+
  labs(title ="Expanded Codes based on D&N:
  ...
  scale_fill_manual(values = c( "#66CC99","#66AA88","gray")) +
  coord_flip()
...
table %>% filter(occ_2dig_labels != "Military") %>%
  ...
  labs(title ="Proportion of All Workers by Broad Occupation type & WFH Status",
       ...
       x = "", y = "# of People",
      caption = "ACS 1 year samples for 2019 and 2021 used for weighted population estimates,") + scale_y_continuous(labels = scales::comma)+
  scale_fill_manual(values = c( "#66CC99","#66AA88","gray")) +  coord_flip()
...
xtabs(~did_wfh+SEX_labels+YEAR, data = joined) %>% summary()
...
# Employment Status
table(joined$EMPSTAT) # unweighted
...
mapPUMAboth <- svytable(~did_wfh_labels+YEAR+PUMA, design = dstrata)
...
mapPUMAboth %>%
  ...
  theme_classic() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank())+
   scale_fill_gradientn(colors = c("white", "gray4"), 
                       limits = c(0,.6),
                       n = 5,
...
mapPUMAboth <- joined %>% 
   ...
   mutate(pct_weight = weightedcount/sum(weightedcount), 
          pct_noweight = unweightedcount/sum(unweightedcount))%>%
  full_join(pumasIL, by = c("PUMA" = "PUMACE10")) 
# table created to view observation count
mapPUMAboth
...
mapPUMAboth %>%
  ...
  labs(title = "Population % Lacking Access to Internet", 
...
mapPUMAboth %>%   
  ...
  theme_minimal()+
    theme(legend.title = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position="right")+
                       ...
                       n = 7,
                        name = "Change in Access", label = scales::percent)+
  labs(title = "Change in Lack of Internet Access", 
       subtitle = "A decrease in Lack of Access is a Good thing",
       caption = "Change in Percentage points: 2021 % - 2019 %.
       Access to internet at home or other location is considered having Access to Internet")
...
## Access to internet in general using survey objects
mapPUMAboth <- svytable(~CINETHH+YEAR+PUMA, design = dstrata)

mapPUMAboth <- mapPUMAboth %>% as_tibble() %>%
filter(CINETHH %in% c(1,2,3)) %>%    
  group_by(YEAR, PUMA) %>% 
     mutate(Prop = round(n/sum(n), digits =3)) %>%
     full_join(pumasIL, by = c("PUMA" = "PUMACE10"))
...
mapPUMAboth %>%
  ...
  ggplot(aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Percent of population w/ Home Internet", 
...
mapPUMAboth %>% 
  ...
  ggplot(aes(fill = pct_change)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Change in Homes w/ Internet",
       subcaption = "2019 to 2021",
       caption = "Percentage point change = 
       2021 PUMA% with CINETHH=1 - 2019 PUMA% with CINETHH=1") +
  scale_fill_gradientn(colors = c("red4",  "white", "forestgreen"), 
                       n = 5,
                       limits = c(-.15,.15),
                        name = "Change in Access", label = scales::percent) + Alea_theme()

mapPUMAboth %>% filter(CINETHH==1)%>%  
  pivot_wider(id_cols = -c(n), names_from ="YEAR", values_from = "Prop") %>% 
    mutate(pct_change = `2021`-`2019`) %>%
  arrange(pct_change) %>% select(PUMA, NAME10, pct_change, `2019`,`2021`)
...
joined %>% filter(CINETHH %in% c(1,2,3)) %>% 
   ...
   dplyr::summarize(weightedcount=sum(PERWT), #weighted 
       ...
       mutate( pct_weight = weightedcount/sum(weightedcount), 
          pct_noweight = unweightedcount/sum(unweightedcount)) %>%
              full_join(pumasIL, by = c("PUMA" = "PUMACE10")) %>% 
  arrange(CINETHH,unweightedcount)

# mapPUMAboth <- joined %>% filter(CINETHH %in% c(1,2,3)) %>% 
#   group_by(county_pop_type, YEAR, PUMA, CINETHH) %>% 
#    dplyr::summarize(weightedcount=sum(PERWT), #weighted 
#                     unweightedcount = n())%>%
#   mutate(pct_weight = weightedcount/sum(weightedcount), 
...
mapPUMAboth %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black")+   
  scale_fill_continuous(low = "white", high = "red4", name = "% of Population", label = scales::percent)+
  labs(title = "Percent of population wwithout Internet", 
       caption = "Geography = PUMAs. 
...
mapPUMAboth %>%
  filter(CINETHH==3)%>%
  select(YEAR,PUMA,geometry,Prop) %>%
  pivot_wider(names_from ="YEAR", values_from = "Prop") %>%
  mutate(pct_change = `2021`-`2019`) %>%
  ggplot(aes(fill = pct_change)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  theme_minimal() +theme(axis.text = element_blank(), axis.ticks = element_blank())+
  labs(title = "Change in % of population w/o access to Internet",
       subtitle = "A decrease is a good thing!",
       caption = "Change = 2021 PUMA% with CINETHH=3 - 2019 PUMA% with CINETHH=3.
       Data from 2019 and 2021 1 year ACS Surveys") +
  scale_fill_gradientn(colors = c("forestgreen",  "white", "red4"), na.value = "grey",
                       n = 5,
                       limits = c(-.15,.15),
                        name = "% PUMA Pop.", label = scales::percent) + Alea_theme()
...
mapPUMAboth %>%
  ...
  labs(title = "Percent of population with internet at home", 
       caption = "Uses CINETHH == 1") + 
  facet_wrap(~YEAR) +
  Alea_theme()+
    theme(legend.title = element_blank())
...
mapPUMAboth %>% 
  ...
  labs(title = "Change in % of PUMA population with access to internet at home",
       ...
       caption = "Percentage point change = 2021 PUMA% with CINETHH=1 - 2019 PUMA% with CINETHH=1.
       Data from 2019 and 2021 1 year ACS Surveys.") +
  scale_fill_gradientn(colors = c("red4", "white", "forestgreen"),
                       limits = c(-.15, .15),
...
# from joined dataframe. Not using survey objects or survey package
mapPUMAboth <- joined %>% 
    filter(CIHISPEED == 10 | CIHISPEED == 20) %>%
...
mapPUMAboth %>%
  ...
  labs(title = "Percent of population without High Speed Internet", subtitle = "Geograph = PUMAs", caption = "Each PUMA has >100K & < 200K people.
       Manually created table without svytable()") + 
  facet_wrap(~YEAR)+
 scale_fill_gradientn(colors = c( "white", "red4"), 
                       n = 7,
                        name = "Population %", label = scales::percent)+Alea_theme()
...
mapPUMAboth <- svytable(~CIHISPEED+YEAR+PUMA, design = dstrata)
...
mapPUMAboth %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Percent of population without High Speed Internet", subtitle = "Geograph = PUMAs", caption = "Each PUMA has >100K & < 200K people.
       Made with Survey Object and svytable()") + 
  facet_wrap(~YEAR)+
 scale_fill_gradientn(colors = c( "white", "red4"), 
                       n = 7,
                        name = "Population %", label = scales::percent)+Alea_theme()
...
mapPUMAboth %>% filter(CIHISPEED==10)%>%  
  pivot_wider(id_cols = -c(n), names_from ="YEAR", values_from = "Prop") %>% 
    mutate(pct_change = `2021`-`2019`) %>%
  arrange(pct_change) %>% select(NAME10, pct_change, PUMA, `2019`,`2021`)
...
mapPUMAboth %>%
  ggplot(aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Percent of population w/o Internet", 
...
mapPUMAboth <- svytable(~CIHISPEED+YEAR+countyFIP, design = dstrata)
...
Graphs below calculate the average for rural areas and add it to the graph so that they are not all gray.

> Check calculations for average for rural counties.
...
mapPUMAboth %>%
  ...
  ggplot(aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Percent of population that Worked from Home", 
       caption = "Geography = Counties with populations > ~60K") + 
  facet_wrap(~YEAR)  +
    theme(legend.title = element_blank())+
...
mapPUMAboth %>% pivot_wider(id_cols = -c(n), names_from ="YEAR", values_from = "Prop") %>%
  ...
  geom_sf(aes(geometry = geometry), color = "black")+ Alea_theme()+ 
   scale_fill_gradientn(colors = c("red4", "white", "forestgreen"), 
                       ...
                       limits = c(-.20,.20),
                        name = "Population %", label = scales::percent)+
  labs(title = "Change in % Pop that Did Work from Home")
```
